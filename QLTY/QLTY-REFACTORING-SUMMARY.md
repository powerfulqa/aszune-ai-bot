...
Refactored `web-dashboard.js` to significantly reduce complexity and improve code quality compliance
with qlty thresholds (max function complexity: 10, file complexity: 15).
...
- **New Methods**:
  ...
  - `_sendErrorResponse(res, error, context, statusCode)` - Standard error with ErrorHandler
    integration
...
- **Extracted Methods**:
  - `_createRecommendation(category, severity, message, action)` - Single source for recommendation
    format
  - Renamed `addXxxRecommendations()` → `_addXxxRecommendations()` for consistency
- **Result**: Reduced getDetailedRecommendations() complexity by using helper method for each
  recommendation push
...
1. `src/services/web-dashboard.js` - Main refactoring (removed 500+ lines of network code, added 4
   helper methods)
