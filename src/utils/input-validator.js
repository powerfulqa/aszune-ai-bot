...
class InputValidator {
  ...
  static validateUserId(userId) {
    try {
      ...
      const errorResponse = ErrorHandler.handleError(error, 'validating user ID', { userId: typeof userId });
  ...
  static escapeHtml(text) {
    try {
      ...
      const errorResponse = ErrorHandler.handleError(error, 'escaping HTML', { textLength: text?.length || 0 });
