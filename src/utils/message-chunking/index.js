...
const { preprocessMessage, fixChunkBoundaries, validateChunkBoundaries } = require('./chunk-boundary-handler');
...
function enhancedChunkMessage(message, maxLength = config.MESSAGE_LIMITS.DISCORD_MAX_LENGTH) {
  try {
    ...
    try {
      ...
      const fallbackErrorResponse = ErrorHandler.handleError(fallbackError, 'fallback message chunking', { 
        messageLength: message?.length || 0,
        maxLength
      });
...
function formatUrls(text) {
  try {
    ...
    const errorResponse = ErrorHandler.handleError(error, 'formatting URLs', { textLength: text?.length || 0 });
...
function getChunkingStats(chunks) {
  try {
    ...
    const errorResponse = ErrorHandler.handleError(error, 'getting chunking stats', { chunkCount: chunks?.length || 0 });
