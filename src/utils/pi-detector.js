      ...
      if (os.platform() === 'linux') {
        ...
        if (isPiByHardware || isPiByModel || isPiByRevision) {
          ...
          try {
            ...
            const tempOutput = execSync(tempCommand, { encoding: 'utf8' });
...
async function initPiOptimizations() {
  try {
    ...
    Object.keys(envOverrides).forEach(key => {
      if (process.env[`PI_${key}`] !== undefined || 
    ...
    if (optimizedConfig.ENABLED) {
      logger.info(`Detected ${piInfo.model} with ${piInfo.ram.toFixed(1)}GB RAM and ${piInfo.cores} cores`);
