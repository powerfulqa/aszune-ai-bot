...
describe('Connection Throttler', () => {
  ...
  describe('executeRequest', () => {
    ...
    it('should handle errors in requests', async () => {
      ...
      await expect(connectionThrottler.executeRequest(mockRequest, 'ERROR')).rejects.toThrow(testError);
      
      expect(mockRequest).toHaveBeenCalledTimes(1);
      expect(logger.error).toHaveBeenCalledWith(expect.stringContaining('Error in ERROR request'), testError);
  ...
  describe('clearQueue', () => {
    it('should clear all queued requests', () => {
      ...
      expect(logger.info).toHaveBeenCalledWith(expect.stringContaining('Cleared 3 pending requests'));
    });
  });
});
