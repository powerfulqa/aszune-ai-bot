...
describe('Bot integration', () => {
  ...
  beforeEach(() => {
    ...
    const messageCreateCall = mockOn.mock.calls.find((call) => call[0] === 'messageCreate');
  ...
  it('handles a normal message and replies', async () => {
    ...
    try {
      ...
      expect(message.reply).toHaveBeenCalledWith({
        embeds: [expect.objectContaining({ description: 'Hi there!' })],
      });
    } finally {
  ...
  it('replies to !summary with history', async () => {
    ...
    request.mockResolvedValueOnce(
      mockSuccessResponse({ choices: [{ message: { content: 'General Kenobi!' } }] })
    );
    await messageCreateHandler(message);
    // The first reply is an embed
    expect(message.reply).toHaveBeenCalledWith({
      embeds: [expect.objectContaining({ description: 'General Kenobi!' })],
    });
    ...
    request.mockResolvedValueOnce(
      mockSuccessResponse({ choices: [{ message: { content: 'A summary of the conversation.' } }] })
    );
  ...
  it('adds emoji reactions for keywords', async () => {
    ...
    const { mockSuccessResponse } = require('../utils/undici-mock-helpers');
    request.mockResolvedValueOnce(
      mockSuccessResponse({ choices: [{ message: { content: 'Indeed it is!' } }] })
    );
    await messageCreateHandler(message);
  ...
  it('adds multiple emoji reactions for multiple keywords', async () => {
    ...
    const { mockSuccessResponse } = require('../utils/undici-mock-helpers');
    request.mockResolvedValueOnce(
      mockSuccessResponse({ choices: [{ message: { content: 'Feelings...' } }] })
    );
    await messageCreateHandler(message);
    expect(message.react).toHaveBeenCalledWith('😊');
    expect(message.react).toHaveBeenCalledWith('😢');
  ...
  it('rate limits user messages', async () => {
    ...
    try {
      ...
      // The first reply is an embed
      expect(message.reply).toHaveBeenCalledWith({
        embeds: [expect.objectContaining({ description: 'response 1' })],
      });
  ...
  it('truncates very long conversation history', async () => {
    ...
    try {
      ...
      const { mockSuccessResponse } = require('../utils/undici-mock-helpers');
      request.mockResolvedValueOnce(
        mockSuccessResponse({ choices: [{ message: { content: 'Hi there!' } }] })
      );
