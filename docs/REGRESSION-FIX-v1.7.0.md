...
regression and clarity issues were identified and addressed without changing any public APIs.
This document records ONLY the incremental fixes – it does not duplicate the full release notes.
...
- **Dashboard Warning Normalisation** (`fix: Dashboard warnings, memory status clarity, server count accuracy, and Pi detector`)
...
- **Undici / Web API Test Compatibility** (`fix: Add File API mock for undici compatibility in tests`)
	- Added lightweight `global.File` mock in `jest.setup.js` so undici-based code paths can run in Jest without external polyfills
