...
class LicenseGenerator {
  ...
  generateLicenseKey(type = 'personal') {
    if (!this.licenseTypes[type]) {
      throw new Error(`Invalid license type: ${type}. Valid types: ${Object.keys(this.licenseTypes).join(', ')}`);
    }
...
**Status:** Active ✅`;
  ...
  displayLicenses(licenses, type) {
    ...
    licenses.forEach((license, i) => {
      ...
      console.log(`   Max Servers: ${license.allowedServers === -1 ? 'Unlimited' : license.allowedServers}`);
